...
def crop_and_warp(img, crop_rect):
	...
	# Performs the transformation on the foto_origem image
...
def find_largest_feature(inp_img, scan_tl=None, scan_br=None):
	...
	"""
	img = inp_img.copy()  # Copy the image, leaving the foto_origem untouched
...
def grid_pronto_para_AI(path):
    
    #Comando do OpenCV que le a imagem recebi pelo usuario
    foto_origem = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
    processed = pre_process_image(foto_origem)
    
    #Pode-se dar print do resultado do threshold
    #cv2.imshow('processed', processed_img)
    
    #Encontram-se os cantos da do grid
    cantos = encontrar_cantos_maior_poligno(processed)
    cropped = crop_and_warp(foto_origem, cantos)
    
    #Pode-se dar print do resultado do cropped
    #cv2.imshow('cropped', cropped_img)
    
    squares = decide_grid(cropped)
    #print(squares)
    digits = get_digits(cropped, squares, 28)
    #print(digits)
    imagem_final = show_digits(digits)
    return imagem_final

def extract_sudoku(image_path):
    imagem_final = grid_pronto_para_AI(image_path)
    return imagem_final
